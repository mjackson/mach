...
function statusHasContent(status) {
  return utils.STATUS_WITHOUT_CONTENT[status] !== true;
}

utils.SAFE_REQUEST_METHODS = {
  GET: true,
  HEAD: true,
  OPTIONS: true,
  TRACE: true
...
function isSafeRequestMethod(method) {
  return utils.SAFE_REQUEST_METHODS[method.toUpperCase()] === true;
}
...
utils.makeHash = makeHash;
function makeHash(string) {
  return crypto.createHash('sha1').update(string).digest('hex');
}

utils.makeKey = makeKey;
...
function setCookie(headers, name, options) {
  var cookie = utils.encodeCookie(name, options);
...
function makeChecksum(file, algorithm) {
