...
/**
 * A high-level method that parses all parameters given in this request and filters them
 * according to the names and filter functions given in the filterMap. This provides users
 ...
 *       // if they were provided by the client. params won't contain any other properties.
 *     });
 *   }
 */
Request.prototype.filterParams = function (filterMap, maxLength, uploadPrefix) {
  return this.parseParams(maxLength, uploadPrefix).then(function (params) {
    var filteredParams = {};
...
/**
 * Returns a promise for an object that is the union of data contained in the request query
 * and body. This can be useful when you want to parse request parameters ad hoc for one
 * particular route. Otherwise, if you know you want to parse request parameters for all
 * routes in your application you should use the mach.params middleware. Please note however
 * that, unlike mach.params, this method does not automatically handle errors with a 413.
 *
 *   var maxUploadLimit = Math.pow(2, 20); // 1 mb
 *
 *   function app(request) {
 *     return request.parseParams(maxUploadLimit).then(function (params) {
 ...
 */
Request.prototype.parseParams = function (maxLength, uploadPrefix) {
