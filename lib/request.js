...
function Request(options) {
  ...
  // These variables should be available to tweak per instance.
  this.maxContentLength = Request.maxContentLength;
  this.uploadDir = Request.uploadDir;
  this.uploadPrefix = Request.uploadPrefix;
 ...
 * to 0 to disable.
 */
Request.maxContentLength = 1048576; // 1M

/**
 * The directory to use to store temporary files created by the default
 * file upload handler.
 */
Request.uploadDir = '/tmp';
...
Request.prototype.parseContent = function () {
  if (!this._parsedContent) {
    if (!this.canParseContent) {
      ...
      this._parsedContent = utils.bufferStream(this.content, this.maxContentLength).then(function (buffer) {
