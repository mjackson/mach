...
/**
 * This is a low-level function responsible for handling a new multipart.Part
 * when parsing multipart request bodies. By default it streams files to disk
 * and uses in-memory buffers for other request parameters. It should be
 * overridden if custom handling (such as streaming directly to a network file
 * storage) is needed.
 */
Request.prototype.handlePart = function (part, uploadPrefix) {
  if (part.isFile) {
    var temporaryPath = makeTemporaryPath(uploadPrefix);
    return streamToDisk(part, temporaryPath);
  }

  return utils.bufferStream(part);
};
...
function parseMultipart(content, maxLength, boundary, partHandler) {
  ...
  content.on('end', function () {
    ...
    var promise = when.all(paramValues).then(function (paramValues) {
      var params = {};
...
function streamToDisk(part, path) {
  var store = {
    path: path,
    name: part.filename,
    type: part.type,
  ...
  var stream = fs.createWriteStream(store.path);
  var value = when.defer();
