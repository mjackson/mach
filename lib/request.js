...
function Request(options) {
  ...
  var content = options.content || NO_CONTENT;
  if (content instanceof Readable) {
    this.content = content;
  } else {
    this.content = makeReadable(content);
  }
}
...
Request.prototype.call = function (app) {
  ...
  return q.when(value, function (response) {
    ...
    // Support returning strings/numbers for convenience.
...
function makeReadable(content) {
  if (typeof content === 'string') content = new Buffer(content, arguments[1]);
  if (!Buffer.isBuffer(content)) throw new Error('Content must be Buffer or string');
