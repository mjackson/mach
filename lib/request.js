 ...
 *   - highWaterMark      The maximum length (in bytes) to use for the content
 *                        stream. Defaults to Request.highWaterMark.
...
function Request(options) {
  ...
  this.highWaterMark = options.highWaterMark || Request.highWaterMark;
  ...
  this.content = new Content(source, { highWaterMark: this.highWaterMark });
}

/**
 * The default maximum length (in bytes) to use for the content buffer of new
 * Requests. Same as Stream.Readable's highWaterMark option.
 */
Request.highWaterMark = Math.pow(2, 14); // 16k
...
Request.prototype.call = function (app) {
  ...
  return q.when(value, function (response) {
    ...
    if (!(response.content instanceof Content)) {
      var content = new Content(response.content);
      response.content = content;
      if (content.hasFixedLength) {
...
Request.prototype.parseContent = function (maxLength) {
  ...
  if (!this.canParseContent) {
    ...
    if (!match) {
      ...
      // TODO: Test this to make sure it works well with streams2.
