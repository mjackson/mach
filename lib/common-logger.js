...
/**
 * A middleware that logs the request to the given stream on its way out in
 * Apache httpd's Common Log Format (see http://httpd.apache.org/docs/1.3/logs.html#common).
 */
function commonLogger(app, stream) {
  ...
  return function (request) {
    return request.call(app).then(function (response) {
      ...
      var logEntry = [host, id, user, timestamp, info, response.status, length].join(' ');
