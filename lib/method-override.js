module.exports = makeMethodOverride;
 ...
 * available.
 */
function makeMethodOverride(app, paramName, headerName) {
  headerName = (headerName || 'X-Http-Method-Override').toLowerCase();
  paramName = paramName || '_method';

  function methodOverride(request) {
    var method;
    if (request.headers[headerName]) {
      method = request.headers[headerName];
    } else if (!request.params) {
      request.error.write('No request params. Use mach.requestParams in front of mach.methodOverride\n');
