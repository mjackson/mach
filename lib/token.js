 ...
 *   </form>
 *
 * On the backend, you need to put both mach.session and mach.requestParams in
 * front of this middleware in order for it to be able to retrieve values from
 * the request session and parameters, like this:
 *
 ...
 *   });
 */
function makeToken(app, paramName, sessionKey, byteLength) {
  byteLength = byteLength || 32;
  sessionKey = sessionKey || '_token';
  paramName = paramName || '_token';

  function tokenApp(request) {
    var session = request.session;
    if (!session) {
      request.error.write('No session. Use mach.session in front of mach.token\n');
      session = {};
    }
    ...
    if (!params) {
      request.error.write('No params. Use mach.requestParams in front of mach.token\n');
