...
/**
 * Returns true if the given object is a valid response.
 */
mach.isResponse = isResponse;
function isResponse(object) {
  return object && ('status' in object) && ('headers' in object) && ('content' in object);
}
...
function bindAppToNodeServer(app, nodeServer) {
  nodeServer.on('request', function (nodeRequest, nodeResponse) {
    ...
    request.call(app).fail(function (error) {
      request.error.write((error.stack || error) + '\n');
      return mach.isResponse(error) ? error : utils.internalServerError();
