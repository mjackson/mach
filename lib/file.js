 ...
 */
function makeFileServer(app, options) {
...
function findFile(file) {
  return statFile(file).then(null, handleStatError);
}
...
function makeEtag(file) {
  return utils.makeChecksum(fs.createReadStream(file));
}
...
function sendFile(file, stat, useLastModified, useEtag) {
  ...
  if (useEtag) {
    return makeEtag(file).then(function (etag) {
